...
void Overlay::paintEvent(QPaintEvent *pe)
{
    ...
    auto xbounds = QString("X: %1 ... %2").arg(m_parent->modelLowerBounds().x(), 0, 'f', 3)
        .arg(m_parent->modelUpperBounds().x(), 0, 'f', 3);
    auto ybounds = QString("Y: %1 ... %2").arg(m_parent->modelLowerBounds().y(), 0, 'f', 3)
        .arg(m_parent->modelUpperBounds().y(), 0, 'f', 3);
    auto zbounds = QString("Z: %1 ... %2").arg(m_parent->modelLowerBounds().z(), 0, 'f', 3)
        .arg(m_parent->modelUpperBounds().z(), 0, 'f', 3);
    auto ranges = QString("%1 / %2 / %3").arg(m_parent->modelRanges().x(), 0, 'f', 3)
        .arg(m_parent->modelRanges().y(), 0, 'f', 3).arg(m_parent->modelRanges().z(), 0, 'f', 3);
    auto vertices = QString(tr("Vertices: %1")).arg(m_parent->vertexCount());
    auto fps = QString("FPS: %1").arg(m_parent->fps());
    auto estimate = m_parent->spendTime().toString("hh:mm:ss") + " / " + m_parent->estimatedTime().toString("hh:mm:ss");
    auto buffer = m_parent->bufferState();

    double x = 10;
    double y = this->height() - fm.height() * 3 - 10;

    // painter.setBrush(QBrush(QColor(255, 255, 255, 196)));
    // painter.drawRect(5, 5, fm.horizontalAdvance(m_parent->parserStatus()) + 10, fm.height() * 3 + 10);
    ...
    painter.drawText(QPoint(x, fm.height() + 10), m_parent->parserStatus());
    painter.drawText(QPoint(x, fm.height() * 2 + 10), m_parent->speedState());
    painter.drawText(QPoint(x, fm.height() * 3 + 10), m_parent->pinState());
    ...
    w.translate(1, -1);
    w *= m_parent->projectionMatrix() * m_parent->viewMatrix();

    foreach (ShaderDrawable *drawable, m_parent->shaderDrawables()) {
        if (drawable->visible()) {
            painter.save();
            drawable->drawPainter(painter, w * drawable->modelMatrix(), height() / m_parent->windowSizeWorld());
            painter.restore();
        }
    }
}
